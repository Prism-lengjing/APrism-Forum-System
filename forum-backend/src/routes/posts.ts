...
router.get(
  ...
  (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      return sendSuccess(res, result, '获取帖子列表成功');
    } catch (error) {
      return next(error);
    }
  }
...
router.post(
  ...
  (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      return sendSuccess(res, result, '回复成功', 201);
    } catch (error) {
      return next(error);
    }
  }
...
router.post(
  ...
  (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = postService.likePost(postId);
      return sendSuccess(res, result, '点赞成功');
    } catch (error) {
      return next(error);
    }
  }
